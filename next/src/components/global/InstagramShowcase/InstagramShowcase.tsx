...
import styles from './InstagramShowcase.module.scss';
import type { InstagramShowcaseTypes } from './InstagramShowcase.types';

export default function InstagramShowcase({ index, heading, list }: InstagramShowcaseTypes) {
  const Heading = index === 0 ? Markdown.h1 : Markdown.h2;

  return (
    <section className={styles['InstagramShowcase']}>
      <header>
      ...
      <ul className={styles.list}>
        {list.map(({ url, img }, i) => (
          <li className={styles.item} key={i}>
            <a href={url} target='_blank' rel='noreferrer' aria-label='Przejdź do zdjęcia'>
              <Img data={img} sizes='(max-width: 899px) 30vw, 450px' />
            </a>
          </li>
        ))}
      </ul>
